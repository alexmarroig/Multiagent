    ...
    <div className="min-h-screen bg-neutral-950 text-neutral-100">
      <aside className="fixed inset-y-0 left-0 z-30 w-[240px] border-r border-neutral-800 bg-neutral-950/95 px-4 py-6 backdrop-blur">
        <div className="mb-10 px-3">
          ...
          <p className="mt-2 text-lg font-semibold tracking-tight">Enterprise Console</p>
        </div>
          ...
          {navigation.map((item) => (
            <Link
              ...
              className={`block rounded-lg px-3 py-2 text-sm transition-all duration-300 ${
                item.active ? 'bg-neutral-900 text-white' : 'text-neutral-400 hover:bg-neutral-900 hover:text-neutral-200'
              }`}
            >
              {item.label}
            </Link>
      ...
      <header className="fixed left-[240px] right-0 top-0 z-20 h-16 border-b border-neutral-800 bg-neutral-950/95 backdrop-blur">
        <div className="mx-auto flex h-full w-full max-w-7xl items-center justify-between px-6">
          <div>
            ...
            <p className="text-lg font-medium tracking-tight text-neutral-100">Dashboard executivo</p>
          </div>
          <div className="rounded-lg border border-neutral-800 bg-neutral-900 px-3 py-1.5 text-xs text-neutral-300">Live</div>
        </div>
      </header>
