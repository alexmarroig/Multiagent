...
function mapAgentNameToTemplate(agentName: string): AgentTemplate {
  const normalized = agentName.toLowerCase();
  const exact = AGENT_TEMPLATES.find((t) => normalized.includes(t.name.toLowerCase().replace('agent', '')));
  if (exact) return exact;
  return AGENT_TEMPLATES.find((t) => t.id === 'supervisor-agent') ?? AGENT_TEMPLATES[0];
}
...
export default function TemplateGallery({ onClose, onTemplateApplied }: TemplateGalleryProps) {
  ...
  const fallbackTemplates = useMemo<Template[]>(
    () => [
      ...
      },
      {
        ...
        description: 'research → marketing → excel → phone',
        agents: ['research', 'marketing', 'excel', 'phone'],
        color: 'blue',
  ...
  const applyTemplate = (template: Template) => {
    ...
    const generatedNodes: Node<AgentNodeData>[] = template.agents.map((agent, index) => {
